...
//***********************************************FCM_S2 Cheng Zhang Median*******************************//

void sort(int ar[9],int n)
{
	int temp,j,i;
	for(i=1;i<n;i++)
	{
		temp=ar[i];                       //insertion sort to calculate median
		j=i-1;
		while(j>=0 && ar[j]>temp)
		{
			ar[j+1]=ar[j];
...
int main()
{
	...
	FILE *fp1 = fopen("saturn.ascii.txt","r");
	FILE *fp2 = fopen("saturn.ascii_FCM_S2.pgm","wb");
   ...
   for(i=1;i<=r;i++) for(j=1;j<=c;j++) fscanf(fp1,"%d",&a[i][j]); //to read the input image
	//*************************matrix padding************************************//
                 ...
                 for(d=0;d<k;d++)
                  {
					...
					for(q=0;q<Sr;q++)
                       scar+=(float)(pow((win[4]-center[d]),2));

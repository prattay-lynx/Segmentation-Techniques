...
int main()
{
	...
	FILE *fp1 = fopen("m12_gray_hist.txt","r");
	FILE *fp2 = fopen("m12_gray_hist_woa.pgm","wb");
...
//    printf("\nCount: %d",count);
   ...
   for(i=0;i<r;i++)
	...
	//**************************************Uniformity calculation**************************************//
		float temp41=0.0,temp42=0.0;
		float *mean=NULL;
		mean=(float*)calloc(k,sizeof(float));
...
//		    printf("\nCount: %d",count1[d]);	
		...
		{   
//		    printf("\nCount: %d",count1[d]);
			mean[d]=(float)count1[d]/(float)(r*c);	
		}
//		printf("\nmean: %.2f ",mean);
		float U;
		float temp8=0.0;
	    for(d=0;d<k;d++)
	    {
			for(i=0;i<r;i++)
			{
				for(j=0;j<c;j++)
				{
					temp8+=(float)pow((b[i][j]-mean[d]),2);
				}
			}
		}
//		U=fabs((1-(2*k*temp8))/((r*c)*pow((f_max),2)));
        U=(float)(f_max-f_min)/(float)(f_max+f_min);
		printf("\nUniformity of the segmented image: %.2f ",U);

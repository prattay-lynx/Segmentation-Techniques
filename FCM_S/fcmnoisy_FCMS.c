...
int main()
{
	...
	FILE *fp2 = fopen("original_lena_FCMS.pgm","wb");
					   ...
					   for(j=1;j<=c;j++)
					      {
			        		for(in=-1;in<2;in++)
			           	  		for(jn=-1;jn<2;jn++)
			           	  			Xpsum+=(a[i+in][j+jn]);     // printf("%f ",Xpsum);
                 ...
                 for(d=0;d<k;d++)
                  {
					...
					for(q=0;q<Sr;q++)
                       scar+=(float)(pow((win[q]-center[d]),2));
